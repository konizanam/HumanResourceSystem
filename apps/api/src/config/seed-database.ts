...
async function tryInsertApplication(
  ...
  try {
    ...
    // If the DB is on an older schema, fall back.
  ...
  try {
    ...
    for (const cat of categories) {
      ...
      for (const subcat of cat.subcategories) {
        ...
        // Remove ON CONFLICT
        await client.query(
          'INSERT INTO job_subcategories (id, category_id, name) VALUES ($1, $2, $3)',
          [subcatId, catId, subcat]
        );
      }
    }
      ...
      try {
        await client.query(
          `INSERT INTO jobs (
            ...
            experience_level, created_by, employer_id, created_at, updated_at, views, applications_count,
            is_featured, is_urgent, requirements, responsibilities, benefits, is_flagged, flag_reason
...
// For each job seeker, apply to 5-8 random jobs
for (const userId of jobSeekerIds) {
  // Shuffle jobs array
  const shuffledJobs = [...jobIds].sort(() => 0.5 - Math.random());
  
  // Pick 5-8 jobs for this user
  const numApplications = Math.min(
    randomInt(5, 8),
    jobIds.length // Don't exceed total jobs
  );
  ...
  for (let i = 0; i < numApplications; i++) {
    ...
    const appliedAt = randomDate(new Date(Date.now() - 25 * 24 * 60 * 60 * 1000), new Date());
    
    try {
      await tryInsertApplication(client, {
        id: applicationId,
        jobId,
        applicantId: userId,
        status,
      ...
      if (Math.random() > 0.6) {
        ...
        try {
          const jobResult = await client.query(
            'SELECT employer_id, created_by FROM jobs WHERE id = $1',
            [jobId]
          );

          if (jobResult.rows.length > 0) {
            const employerId = jobResult.rows[0].employer_id ?? jobResult.rows[0].created_by;
            if (employerId) {
              await client.query(
                ...
                [
                  ...
                  applicationId,
                  employerId,
                  `Application updated. ${Math.random() > 0.5 ? 'Reviewed by employer.' : 'Pending review.'}`,
          ...
          // Best-effort; notes table may not exist across schema variants.
        }
      }
    } catch (err: any) {
      if (err.code === '23505') {
        ...
        if (shuffledJobs[i]) {
          await client.query(
            `INSERT INTO saved_jobs (user_id, job_id, saved_at) 
             VALUES ($1, $2, $3)`,
            [userId, shuffledJobs[i], randomDate(new Date(2024, 0, 1), new Date())]
          );
        }
      }
    }
...
console.log('📝 Continuing with permissions...');
    ...
    for (const perm of permissions) {
      ...
      await client.query(
        `INSERT INTO permissions (id, name, description, module_name, action_type) 
         VALUES ($1, $2, $3, $4, $5)`,
        [permId, perm.name, perm.description, perm.module, perm.action]
      );
    }
    ...
    for (let i = 0; i < 30; i++) {
      ...
      await client.query(
        ...
        [
          ...
          `192.168.${randomInt(1, 254)}.${randomInt(1, 254)}`,
          'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
          randomDate(new Date(Date.now() - 14 * 24 * 60 * 60 * 1000), new Date())
        ]
      );
    }
