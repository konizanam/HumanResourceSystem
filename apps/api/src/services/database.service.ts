...
export class DatabaseService {
  ...
  async assignJobSeekerRole(userId: string): Promise<void> {
    // Get JOB_SEEKER role ID
    const roleResult = await query(
      "SELECT id FROM roles WHERE name = 'JOB_SEEKER'"
    );
  ...
  async getJobSeekerProfile(userId: string): Promise<JobSeekerProfile | null> {
    const result = await query(
