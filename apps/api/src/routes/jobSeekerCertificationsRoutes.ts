...
const validateCertification = [
  ...
  body('issuing_organization').notEmpty().withMessage('Issuing organization is required').trim(),
  body('issue_date').isISO8601().toDate().withMessage('Valid issue date is required'),
  body('expiration_date').optional().isISO8601().toDate(),
  body('credential_id').optional().isString().trim(),
  body('credential_url').optional().isURL().withMessage('Valid URL is required'),
  body('does_not_expire').optional().isBoolean().toBoolean(),
...
router.get('/',
  ...
  [
    ...
    query('sort').optional().isIn(['newest', 'oldest', 'expiring_soon'])
  ],
...
router.delete('/:id',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await dbQuery('DELETE FROM certifications WHERE id = $1', [certificationId]);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const checkResult = await dbQuery(
        'SELECT id FROM certifications WHERE id = $1 AND job_seeker_id = $2',
        [certificationId, jobSeekerId]
      );
