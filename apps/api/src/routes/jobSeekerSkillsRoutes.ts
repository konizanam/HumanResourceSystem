...
const validateSkill = [
  ...
  body('years_of_experience').optional().isInt({ min: 0, max: 50 }).toInt(),
    ...
    try {
      ...
      const countsResult = await dbQuery(
        `SELECT 
         ...
         FROM skills WHERE job_seeker_id = $1`,
        [jobSeekerId]
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const existingSkill = await dbQuery(
        'SELECT id FROM skills WHERE job_seeker_id = $1 AND LOWER(name) = LOWER($2)',
        [jobSeekerId, name]
      );
      ...
      if (is_primary) {
        await dbQuery(
          'UPDATE skills SET is_primary = FALSE WHERE job_seeker_id = $1',
          [jobSeekerId]
        );
      }
      ...
      const result = await dbQuery(
        ...
        RETURNING *`,
        [jobSeekerId, name, proficiency_level, years_of_experience, is_primary]
      );
...
router.delete('/:id',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const skillResult = await dbQuery(
      ...
      // Delete skill
      await dbQuery('DELETE FROM skills WHERE id = $1', [skillId]);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const result = await dbQuery(
          'UPDATE skills SET is_primary = TRUE, updated_at = NOW() WHERE id = $1 RETURNING *',
          [skillId]
        );
