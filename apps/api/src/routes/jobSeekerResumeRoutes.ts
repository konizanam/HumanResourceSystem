  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        if (isPrimary) {
          await dbQuery(
            'UPDATE resumes SET is_primary = FALSE WHERE job_seeker_id = $1',
            [jobSeekerId]
          );
        }
    ...
    try {
      ...
      try {
        ...
        if (resume.is_primary) {
          const otherResume = await dbQuery(
          ...
          if (otherResume.rows.length > 0) {
            await dbQuery(
              'UPDATE resumes SET is_primary = TRUE WHERE id = $1',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        // Set new primary
        await dbQuery(
          'UPDATE resumes SET is_primary = TRUE, updated_at = NOW() WHERE id = $1',
          [resumeId]
        );
