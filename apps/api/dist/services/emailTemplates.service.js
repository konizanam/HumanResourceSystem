...
exports.getDefaultEmailTemplates = getDefaultEmailTemplates;
exports.getEmailTemplates = getEmailTemplates;
exports.updateEmailTemplate = updateEmailTemplate;
exports.isEmailTemplateKey = isEmailTemplateKey;
exports.isDefaultEmailTemplateKey = isDefaultEmailTemplateKey;
exports.parsePlaceholdersList = parsePlaceholdersList;
...
function storageFilePath() {
    ...
    if (process.env.EMAIL_TEMPLATES_PATH) {
        return path_1.default.resolve(process.env.EMAIL_TEMPLATES_PATH);
    }
    ...
    return path_1.default.resolve(__dirname, '..', '..', 'data', 'email-templates.json');
}
async function readStoredFile() {
    try {
        const raw = await promises_1.default.readFile(storageFilePath(), 'utf8');
        ...
        if (parsed.version === 1 && typeof parsed.templates === 'object' && parsed.templates) {
            const migrated = { version: 2, templates: {} };
...
async function writeStoredFile(file) {
    const filePath = storageFilePath();
    const dir = path_1.default.dirname(filePath);
    await promises_1.default.mkdir(dir, { recursive: true });
    const tmpPath = `${filePath}.tmp`;
    await promises_1.default.writeFile(tmpPath, JSON.stringify(file, null, 2), 'utf8');
...
async function updateEmailTemplate(key, updates) {
    const now = new Date().toISOString();
    const stored = (await readStoredFile()) ?? { version: 2, templates: {} };
    const base = DEFAULT_TEMPLATES[key];
    ...
    const mergedPlaceholders = Array.isArray(stored.templates[key].placeholders)
        ? stored.templates[key].placeholders
        : base?.placeholders ?? [];
    return {
        key,
        title: mergedTitle,
...
function isEmailTemplateKey(v) {
    return typeof v === 'string' && /^[a-z0-9][a-z0-9_-]{2,63}$/.test(v);
}
function isDefaultEmailTemplateKey(key) {
    return Boolean(DEFAULT_TEMPLATES[key]);
}
function parsePlaceholdersList(raw) {
