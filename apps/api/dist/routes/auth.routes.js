...
async function sendLoginNotificationEmail(params) {
    try {
        const dateTime = (0, emailSender_service_1.formatLoginDateTime)(new Date());
        const location = (0, emailSender_service_1.describeIpLocation)(params.ip);
        await (0, emailSender_service_1.sendTemplatedEmail)({
        ...
        console.error("[Email] Failed to send login notification email:", e instanceof Error ? e.message : e);
    }
}
/* ------------------------------------------------------------------ */
/*  Helpers                                                            */
...
/* ------------------------------------------------------------------ */
exports.authRouter.post("/refresh", auth_1.authenticate, async (req, res, next) => {
    try {
       ...
       WHERE u.id = $1 AND u.is_active = TRUE`, [userId]);
        ...
        const roles = Array.from(new Set(rows
