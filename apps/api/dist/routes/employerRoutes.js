...
const validateEmployerRegistration = [
    ...
    (0, express_validator_1.body)('company_name').notEmpty().withMessage('Company name is required').trim(),
    (0, express_validator_1.body)('company_description').optional().isString().trim(),
    (0, express_validator_1.body)('company_website').optional().isURL().withMessage('Valid website URL is required'),
    (0, express_validator_1.body)('company_size').optional().isIn(['1-10', '11-50', '51-200', '201-500', '501-1000', '1000+']),
    (0, express_validator_1.body)('industry').optional().isString().trim(),
    (0, express_validator_1.body)('founded_year').optional().isInt({ min: 1800, max: new Date().getFullYear() }),
    (0, express_validator_1.body)('headquarters_location').optional().isString().trim(),
...
const validateEmployerProfile = [
    (0, express_validator_1.body)('company_name').optional().notEmpty().withMessage('Company name cannot be empty').trim(),
    (0, express_validator_1.body)('company_description').optional().isString().trim(),
    (0, express_validator_1.body)('company_website').optional().isURL().withMessage('Valid website URL is required'),
    (0, express_validator_1.body)('company_size').optional().isIn(['1-10', '11-50', '51-200', '201-500', '501-1000', '1000+']),
    (0, express_validator_1.body)('industry').optional().isString().trim(),
    ...
    (0, express_validator_1.body)('headquarters_location').optional().isString().trim(),
    (0, express_validator_1.body)('phone').optional().isMobilePhone('any').withMessage('Valid phone number is required'),
    (0, express_validator_1.body)('company_logo_url').optional().isURL().withMessage('Valid logo URL is required')
];
/**
 * @swagger
 * components:
 ...
 */
router.post('/register', validateEmployerRegistration, async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
        }
        const { email, password, firstName, lastName, company_name, company_description, company_website, company_size, industry, founded_year, headquarters_location, phone } = req.body;
        ...
        try {
            ...
            const userResult = await (0, database_1.query)(`INSERT INTO users (
          ...
          RETURNING id, email, first_name, last_name, company_name, created_at`, [
            ...
            await (0, database_1.query)(`INSERT INTO employer_stats (employer_id, last_active, updated_at)
           VALUES ($1, NOW(), NOW())`, [employer.id]);
 ...
 */
router.get('/profile', auth_1.authenticate, (0, auth_1.authorize)('EMPLOYER', 'ADMIN'), async (req, res) => {
    try {
        const employerId = req.user.userId;
        // Get employer profile
        const profileResult = await (0, database_1.query)(`SELECT id, email, first_name, last_name, 
          company_name, company_description, company_website,
          company_logo_url, company_size, industry, founded_year,
          headquarters_location, phone, is_verified, created_at, updated_at
         FROM users
         WHERE id = $1 AND role = 'EMPLOYER'`, [employerId]);
        if (profileResult.rows.length === 0) {
            return res.status(404).json({ error: 'Employer not found' });
        }
        // Get employer stats
        const statsResult = await (0, database_1.query)(`SELECT * FROM employer_stats WHERE employer_id = $1`, [employerId]);
 ...
 *         description: Employer not found
 */
router.put('/profile', auth_1.authenticate, (0, auth_1.authorize)('EMPLOYER', 'ADMIN'), validateEmployerProfile, async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
        ...
        const employerId = req.user.userId;
        const { firstName, lastName, company_name, company_description, company_website, company_logo_url, company_size, industry, founded_year, headquarters_location, phone } = req.body;
        ...
        const result = await (0, database_1.query)(`UPDATE users 
         ...
         RETURNING id, email, first_name, last_name, 
          company_name, company_description, company_website,
          company_logo_url, company_size, industry, founded_year,
          headquarters_location, phone, is_verified, created_at, updated_at`, values);
        ...
        await (0, database_1.query)('UPDATE employer_stats SET last_active = NOW(), updated_at = NOW() WHERE employer_id = $1', [employerId]);
 ...
 *         description: Forbidden - Employer access required
 */
router.get('/jobs', auth_1.authenticate, (0, auth_1.authorize)('EMPLOYER', 'ADMIN'), [
    (0, express_validator_1.query)('page').optional().isInt({ min: 1 }).toInt(),
    (0, express_validator_1.query)('limit').optional().isInt({ min: 1, max: 100 }).toInt(),
    (0, express_validator_1.query)('status').optional().isIn(['active', 'closed', 'draft']),
    (0, express_validator_1.query)('sort').optional().isIn(['newest', 'oldest', 'most_viewed', 'most_applied'])
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
 ...
 *         description: Forbidden - Employer access required
 */
router.get('/applications', auth_1.authenticate, (0, auth_1.authorize)('EMPLOYER', 'ADMIN'), [
    (0, express_validator_1.query)('page').optional().isInt({ min: 1 }).toInt(),
    (0, express_validator_1.query)('limit').optional().isInt({ min: 1, max: 100 }).toInt(),
    ...
    try {
        ...
        const applicationsResult = await (0, database_1.query)(`SELECT a.*,
        ...
        const summaryResult = await (0, database_1.query)(`SELECT 
         ...
         FROM applications a
         LEFT JOIN jobs j ON a.job_id = j.id
         WHERE j.employer_id = $1`, [employerId]);
        // Format applications
 ...
 *         description: Forbidden - Employer access required
 */
router.get('/dashboard', auth_1.authenticate, (0, auth_1.authorize)('EMPLOYER', 'ADMIN'), async (req, res) => {
    try {
        const employerId = req.user.userId;
        // Get employer profile
        const profileResult = await (0, database_1.query)(`SELECT id, email, first_name, last_name, company_name, company_logo_url
         FROM users WHERE id = $1`, [employerId]);
        // Get employer stats
        const statsResult = await (0, database_1.query)(`SELECT * FROM employer_stats WHERE employer_id = $1`, [employerId]);
        // Get recent jobs (last 5)
        const recentJobsResult = await (0, database_1.query)(`SELECT id, title, status, views, applications_count, created_at
         FROM jobs
         WHERE employer_id = $1
         ORDER BY created_at DESC
         LIMIT 5`, [employerId]);
        // Get recent applications (last 10)
        const recentApplicationsResult = await (0, database_1.query)(`SELECT a.*, j.title as job_title, u.first_name, u.last_name
         FROM applications a
         LEFT JOIN jobs j ON a.job_id = j.id
         LEFT JOIN users u ON a.applicant_id = u.id
        ...
        const appsOverTimeResult = await (0, database_1.query)(`SELECT DATE(a.created_at) as date, COUNT(*) as count
         FROM applications a
         LEFT JOIN jobs j ON a.job_id = j.id
         WHERE j.employer_id = $1
           AND a.created_at >= NOW() - INTERVAL '30 days'
         GROUP BY DATE(a.created_at)
         ORDER BY date`, [employerId]);
        // Get jobs by status
        const jobsByStatusResult = await (0, database_1.query)(`SELECT status, COUNT(*) as count
         FROM jobs
         WHERE employer_id = $1
         GROUP BY status`, [employerId]);
        const jobsByStatus = { active: 0, draft: 0, closed: 0 };
...
function generateJWTToken(userId, email, roles) {
