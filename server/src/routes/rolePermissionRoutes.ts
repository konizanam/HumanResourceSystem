...
// Some routes use :id and others use :roleId.
// Validate whichever param is present.
const validateRoleId = [
  param('id').optional().isUUID().withMessage('Invalid role ID'),
  param('roleId').optional().isUUID().withMessage('Invalid role ID'),
];
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await logAudit({
        ...
        targetId: result.rows[0].id,
        details: { name, module_name, action_type },
      });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await logAudit({
        ...
        details: { name: permCheck.rows[0].name },
      });
