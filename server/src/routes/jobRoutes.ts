...
const validateJob = [
  ...
  body('salary_min').optional({ nullable: true, checkFalsy: true }).isNumeric().withMessage('Minimum salary must be a number'),
  body('salary_max').optional({ nullable: true, checkFalsy: true }).isNumeric().withMessage('Maximum salary must be a number'),
...
router.patch('/:id/applications/:applicationId/status',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const interviewDate = String(req.body.interview_date ?? req.body.interviewDate ?? '').trim();
      const interviewTime = String(req.body.interview_time ?? req.body.interviewTime ?? '').trim();
