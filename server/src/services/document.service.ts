...
export class DocumentService {
  ...
  async associateWithJobSeeker(documentId: string, userId: string, documentType: string, isPrimary: boolean = false) {
    ...
    const result = await query(
      `INSERT INTO job_seeker_documents (user_id, document_id, document_type, is_primary)
       ...
       RETURNING *`,
      [userId, documentId, documentType, isPrimary]
    );
  ...
  async associateWithCompany(documentId: string, companyId: string, documentType: string, isPrimary: boolean = false) {
    // If setting as primary, unset other primary documents of this type
    if (isPrimary) {
      await query(
    ...
    const result = await query(
      `INSERT INTO company_documents (company_id, document_id, document_type, is_primary)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (company_id, document_id) 
       DO UPDATE SET document_type = EXCLUDED.document_type, is_primary = EXCLUDED.is_primary
       RETURNING *`,
      [companyId, documentId, documentType, isPrimary]
    );
  ...
  async updateDocument(documentId: string, userId: string, updates: any) {
    ...
    const result = await query(
      `UPDATE documents 
       ...
       WHERE id = $5
       RETURNING *`,
      [document_type, category, description, is_public, documentId]
    );
  ...
  async setPrimaryJobSeekerDocument(userId: string, documentId: string, documentType: string) {
    ...
    if (result.rows.length === 0) {
      throw new NotFoundError('Document association not found');
    }
